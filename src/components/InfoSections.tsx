...
export function InteractiveEarningsCalculator({ theme }: { theme: 'light' | 'dark' | 'classic' }) {
  const [cartonsCount, setCartonsCount] = React.useState<number>(800);
  const [partnerType, setPartnerType] = React.useState<'reseller' | 'agent' | 'supermarket'>('reseller');

  const getSpecs = () => {
    switch (partnerType) {
      case 'agent':
        return { label: "نماینده رسمی توزیع استانی", avgProfitPerCarton: 58000, marginText: "حاشیه کارمزد ثابت + پاداش تارگت", baseSalary: 15000000 };
      case 'supermarket':
  ...
  const grossProfit = cartonsCount * specs.avgProfitPerCarton;
  const platformBonus = cartonsCount > 1500 ? grossProfit * 0.10 : cartonsCount > 500 ? grossProfit * 0.05 : 0;
    ...
    <div className="p-6 sm:p-8 rounded-[2rem] border border-slate-100 bg-white text-right space-y-6 shadow-sm" dir="rtl">
      <div className="space-y-1">
        <div className="inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-[10px] font-black bg-amber-500/10 text-amber-600 border border-amber-500/20">
          <TrendingUp size={12} />
          محاسبه‌گر پویای سود و بازدهی مالی نمایندگان
        </div>
        <h3 className={`text-lg sm font-black ${theme === 'dark' ? 'text-white' : 'text-slate-900'}`}>پرزنت و شبیه‌ساز مالی طرح درآمدزایی دست اول</h3>
        <p className="text-xs text-slate-400 font-bold">بسته به حجم فروش و نوع فعالیت تجاری خود، سود تضمینی حاصل از حذف واسطه‌ها را برآورد کنید.</p>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-center pt-2">
        <div className="lg:col-span-6 space-y-6">
          <div className="space-y-3">
            <label className="text-xs font-black text-slate-500 block">نوع فعالیت تجاری و نحوه همکاری خود را انتخاب کنید:</label>
            <div className="grid grid-cols-3 gap-2">
              <button
                type="button"
                onClick={() => setPartnerType('reseller')}
                className={`py-2 px-3 rounded-xl text-[10px] sm font-black border transition-all ${
                  partnerType === 'reseller' 
                    ? 'bg-amber-500 border-amber-500 text-slate-950 shadow-md shadow-amber-500/10' 
                    : theme === 'dark' ? 'bg-slate-50 text-slate-400 border-slate-800' : 'bg-white text-slate-600 border-slate-150'
                }`}
              >
                📦 بنکدار پخش
              </button>
              ...
              <button
                type="button"
                onClick={() => setPartnerType('supermarket')}
                className={`py-2 px-3 rounded-xl text-[10px] sm font-black border transition-all ${
          ...
          <div className="space-y-3">
            <div className="flex justify-between items-center text-xs">
              <span className="font-mono text-emerald-600 font-extrabold text-sm">{cartonsCount} کارتن</span>
              <span className="font-black text-slate-500">حجم توزیع ماهانه هدف:</span>
            </div>
            <input
              type="range"
              min="50"
            ...
            <div className="flex justify-between text-[10px] font-bold text-slate-400">
              <span>۵,۰۰۰ کارتن</span>
              <span>۲,۵۰۰</span>
              <span>۵۰ کارتن</span>
            </div>
          </div>

          <div className="p-4 rounded-xl bg-amber-500/5 border border-amber-500/10 text-[11px] font-bold leading-relaxed text-amber-700">
            💡 <span className="font-black">پاداش ویژه حجم فروش:</span> با فروش بیش از ۵۰۰ کارتن در ماه، پاداش ۵ درصدی و بیش از ۱۵۰۰ کارتن، ۱۰ درصد پاداش نقدی مازاد بر سود ثابت به فاکتور شما تعلق می‌گیرد.
          </div>
        </div>
