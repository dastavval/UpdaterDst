...
# Find the MAIN products tab block
prod_start_idx = -1
for i, line in enumerate(lines):
    if "{activeSubTab === 'products' && (" in line and "animate-in" in lines[i+1]:
        prod_start_idx = i
        break
...
if prod_start_idx == -1:
    # Try another way
    for i, line in enumerate(lines):
        if "{activeSubTab === 'products' && (" in line and i > 4000:
            prod_start_idx = i
            break
...
while i < prod_end_idx:
    ...
    if "activeSubTab" in line and any(f"'{name}'" in line for name in to_extract) and "&& (" in line:
        ...
        paren_count = 0
        found_end = False
        for j in range(i, len(lines)): # Look beyond prod_end_idx just in case
            lt = lines[j]
            for char in lt:
...
print("AdminPanel.tsx fixed successfully (v3).")
